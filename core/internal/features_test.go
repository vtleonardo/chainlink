...
func TestIntegration_MultiwordV2(t *testing.T) {
	...
	app.Config.Set("ETH_HEAD_TRACKER_MAX_BUFFER_SIZE", 100)
	app.Config.Set("MIN_OUTGOING_CONFIRMATIONS", 1)
	// app.Config.Set("TRIGGER_FALLBACK_DB_POLL_INTERVAL", "1000ms")
	...
	fmt.Println("xyzzy addr", operatorAddress.Hex())

	var jobID [32]byte
	copy(jobID[:], j.ExternalJobID.Bytes())
	_, err = consumerContract.SetSpecID(user, jobID)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	b.Commit()
	r, err := b.TransactionReceipt(context.Background(), tx.Hash())
	require.NoError(t, err)

	bs, _ := json.MarshalIndent(r, "", "    ")
	fmt.Println("xyzzy", string(bs))
	...
	pipelineRuns := cltest.WaitForPipelineComplete(t, 0, j.ID, 1, 11, app.JobORM(), 10*time.Second, 100*time.Millisecond)
